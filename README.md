...
*Generated by `RawDataOverview.py`*
...
The CPC chart above showed why blending groups can hide the real pattern, so I rebuilt the last three plots split by the relevant grouping variable instead of looking at everyone combined.
...
*Generated by `SplitByVariantChannel.py`*
...
**Suggested rollout**: start VarB at 50% of traffic alongside the current form for 2-4 weeks, confirm conversion rate and lead quality hold at the levels seen in testing, then move to 100% once confirmed.
...
!python CPCByChannel.py              # 9. Independent t-test and Levene's test
```
````
